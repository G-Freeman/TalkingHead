...
const EMOTIONS: readonly EmotionPreset[] = [
	{ scale: .7, tilt: 0, brightness: 2, verticalBias: .2 },
	{ scale: 0.5, tilt: 0.18, brightness: 1.15, verticalBias: 0.06 },
	{ scale: 0.6, tilt: -0.12, brightness: 1.05, verticalBias: -0.04 },
	{ scale: 0.8, tilt: 0.05, brightness: 1.5, verticalBias: 0.02 }
];
...
const drawEye = (
	...
	const blinkScale = 1 - blink * 2.92;
	const widthScale = 1 + Math.abs(look.x * 0.22 * side);
	const eyeWidth = width * 0.48 * scale * widthScale;
	const eyeHeight = Math.max(height * 0.21 * scale * blinkScale, height * 0.05);
